...
def ldscore_pairwise(args, log):
    ...
    log.log("Estimating Pairwise LD Score.")
    #lN = geno_array.ldScoreVarBlocks(block_left, args.chunk_size, annot=annot_matrix)
    pairwise_ld_score_output_file = args.out + '_pairwise_l2.txt'
    geno_array.generate_pairwise_lds(block_left, geno_array.df, pairwise_ld_score_output_file)
...
def ldscore(args, log):
    ...
    #ld_scores, pairwise_lds, pairwise_ld_snps = geno_array.generate_pairwise_lds(block_left)

    #temp_ld_scores, temp_pairwise_lds, temp_pairwise_ld_snps = geno_array.generate_pairwise_lds_slow(block_left, block_right)
...
parser.add_argument('--l2-pairwise', default=False, action='store_true',
    help='Estimate l2 for pairwise estimates.')
