...
#NONNEGATIVE_LM = pystan.StanModel('nonnegative_variational_V_update.stan')
#f = open('nonnegative_variational_V_update.pkl', 'wb')
# pickle.dump(NONNEGATIVE_LM, f)
# f.close()
NONNEGATIVE_LM = pickle.load(open('/work-zfs/abattle4/bstrober/temp5/nonnegative_variational_V_update.pkl', 'rb'))
...
class USLDSC(object):
	def __init__(self, K=5, a_u=1.0, b_u=1.0, alpha=1e-3, beta=1e-3, max_iter=1000, ridge_variance=1.0):
		self.K = K
		...
		self.ridge_variance = ridge_variance
	...
	def update_V(self, vi_iter):
		...
		for study_num in range(self.num_studies):
			...
			#opti_data = dict(J=self.num_snps, K=self.K, X=ld_scores, X_squared=squared_ld_scores, y=study_chi_sq, intercept=self.intercept_mu[study_num], precisions=weighted_tau, N=study_sample_size, ridge_variance=self.ridge_variance)
			#op = NONNEGATIVE_LM.optimizing(data = opti_data, verbose=False,iter=5000,seed=1)
			...
			opti_subsampled_data = dict(J=len(subsampled_indices), K=self.K, X=ld_scores[subsampled_indices,:], X_squared=squared_ld_scores[subsampled_indices,:], y=study_chi_sq[subsampled_indices], intercept=self.intercept_mu[study_num], precisions=weighted_tau[subsampled_indices], N=study_sample_size, ridge_variance=self.ridge_variance)
			# WARNING: THIS IS CURRENTLY VERY UNSTABLE AND GIVES DIFFERENT RESULTS DEPDNEING ON INITs
			try:
				op_subsampled = NONNEGATIVE_LM.optimizing(data = opti_subsampled_data, verbose=False,iter=5000)
			except RuntimeError:
				try:
					op_subsampled = NONNEGATIVE_LM.optimizing(data = opti_subsampled_data, verbose=False,iter=5000,seed=1, algorithm="Newton")
				except RuntimeError:
					try:
						op_subsampled = NONNEGATIVE_LM.optimizing(data = opti_subsampled_data, verbose=False,iter=5000,seed=2)
					except RuntimeError:
						print('erororro')
						pdb.set_trace()
			print(op_subsampled['beta'])
			self.V_mu[:, study_num] = op_subsampled['beta']
