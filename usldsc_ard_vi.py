...
class USLDSC(object):
	def __init__(self, K=5, a_u=1.0, b_u=1.0, a_v=1.0, b_v=1.0, alpha=1e-10, beta=1e-10, max_iter=60):
		self.K = K
	...
	def fit(self, chi_squared_files, study_sample_sizes, pairwise_ld_files, pairwise_ld_indices_files, num_snps, cluster_ukbb_files, cluster_pairwise_ld_matrix_files, cluster_variant_names_files, cluster_variant_neighbor_positions_files, output_root):
		...
		self.study_sample_sizes = self.study_sample_sizes
	...
	def update_gamma_U(self, vi_iter):
		...
		if vi_iter > 1:
			self.gamma_U_alpha = self.alpha_prior + np.ones((self.num_snps, self.K))/2.0
	...
	def update_gamma_V(self, vi_iter):
		...
		if vi_iter > 1:
			self.gamma_V_alpha = self.alpha_prior + np.ones((self.K, self.num_studies))
	...
	def update_U(self, vi_iter):
		...
		for cluster_iter in range(self.num_snp_clusters):
			...
			num_cluster_snps = len(cluster_variant_names)
			cluster_chi_squared = np.load(self.cluster_ukbb_files[cluster_iter]).reshape((num_cluster_snps, self.num_studies), order='F')
			predicted_U = (self.U_mu[cluster_variant_names,:])
			...
			for snp_iter in np.random.permutation(range(num_cluster_snps)):
				...
				for kk in range(self.K):
					...
					a_term = (-gamma_U_expected_val[snp_name, kk]/2.0) - (1.0/2.0)*sum_neighbor_ld_squared*sum_squared_Vs[kk] #
	...
	def update_intercept(self):
		...
		V_expected_val = self.V_mu
		# Calculate LD SCORES
		ld_scores = generate_ld_scores(self.U_mu, self.pairwise_ld_files, self.pairwise_ld_indices_files)
	...
	def update_V(self, vi_iter):
		...
		ld_scores, squared_ld_scores = generate_ld_scores_and_squared_ld_scores(self.U_mu, (np.square(self.U_mu) + self.U_var), self.pairwise_ld_files, self.pairwise_ld_indices_files)
		# Compute other useful expectations
		tau_expected_val = self.tau_alpha/self.tau_beta
		gamma_V_expected_val = self.gamma_V_alpha/self.gamma_V_beta
		log_gamma_V_expected_val = special.digamma(self.gamma_V_alpha) - np.log(self.gamma_V_beta)
		# Perform VI updates independently for each study
		for study_num in range(self.num_studies):
			# Extract relevent data for this study
			study_chi_sq = np.load(self.chi_squared_files[study_num])
			...
			for kk in range(self.K):
				...
				a_term = (-gamma_V_expected_val[kk, study_num]/2.0) - (tau_expected_val/2.0)*np.square(study_sample_size)*np.sum(squared_ld_scores[:, kk])

				other_components = np.dot(ld_scores, (self.V_mu[:,study_num])) - ld_scores[:,kk]*(self.V_mu[kk,study_num])
				...
				#ln_1_minus_theta_V_expected_val = special.digamma(self.theta_V_b[kk]) - special.digamma(self.theta_V_a[kk]+self.theta_V_b[kk])
					...
					#z_term = ln_theta_V_expected_val - ln_1_minus_theta_V_expected_val + .5*log_gamma_V_expected_val - .5*np.log(-2.0*a_term) + (np.square(b_term)/(-4.0*a_term))
	...
	def initialize_variables(self):
		...
		self.V_mu = np.random.randn(self.K, self.num_studies)/1000.0
		self.V_var = np.ones((self.K, self.num_studies))/10.0
		...
		self.U_mu = np.random.randn(self.num_snps, self.K)/1000.0
		self.U_var = np.ones((self.num_snps, self.K))/10.0
		...
		# U var
		self.gamma_U_alpha = np.ones((self.num_snps, self.K))
		self.gamma_U_beta = np.ones((self.num_snps, self.K))
		# V var
		self.gamma_V_alpha = np.ones((self.K, self.num_studies))
		self.gamma_V_beta = np.ones((self.K, self.num_studies))
