...
def save_standardized_ukbb_study_data_to_npy(study, chromosome_arr, processed_ukbb_dir, npy_output_file, variant_ids):
	arr = []
	counter = 0
	for chrom_num in chromosome_arr:
		file_name = processed_ukbb_dir + 'chr' + chrom_num + '_' + study + '_chi_squared.txt'
	...
	np.save(npy_output_file, standardized_arr)
...
def pickle_standardized_ukbb_chi_squared_stats(chromosome_arr, processed_ukbb_dir, organized_training_data_dir):
	input_study_file = processed_ukbb_dir + 'updated_ukbb_studies.txt'
	output_study_file = organized_training_data_dir + 'usldsc_training_chromosome_standardized_studies.txt'
	variant_ids = extract_variant_ids(input_study_file, chromosome_arr, processed_ukbb_dir)
	npy_variant_output_file = organized_training_data_dir + 'usldsc_training_chromosome_standardized_ukbb_variant_ids.npy'
	...
	for line in f:
		...
		study = data[0]
		sample_size = '1'
		npy_output_file = organized_training_data_dir + 'usldsc_training_' + study + '_standardized_chi_squared_stats.npy'
		t.write(study + '\t' + npy_output_file + '\t' + sample_size + '\n')
		save_standardized_ukbb_study_data_to_npy(study, chromosome_arr, processed_ukbb_dir, npy_output_file, variant_ids)
	f.close()
	t.close()
...
def pickle_standardized_cluster_level_pairwise_ld_data(chromosome_arr, processed_ld_score_dir, organized_training_data_dir):
	for chromosome in chromosome_arr:
		# Extract pairwise ld info on this chromosome
		raw_pairwise_ld_file = processed_ld_score_dir + 'chr_' + chromosome + '_ld_scores_pairwise_l2_neighbors_filtered.2_clustered.txt'
		neighboring_indices_arr, neighboring_pairwise_lds_arr = extract_chromosome_pairwise_ld_info(raw_pairwise_ld_file)
		...
		for line in f:
			...
			for snp_iter in range(num_snps):
				...
				for neighbor_iter in range(len(snp_neighbor_names)):
					...
					snp_neighbor_positions.append(name_to_position[neighbor_name])
				neighbor_positions_arr.append(np.asarray(snp_neighbor_positions))
			npy_output_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_pairwise_ld_correlation_mat.npy'
			np.save(npy_output_file, pairwise_correlation_mat)
			npy_output_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_pairwise_ld_neighbor_positions.npy'
			np.save(npy_output_file, neighbor_positions_arr)
		f.close()

def pickle_standardized_cluster_level_ukbb_data(chromosome_arr, study_names, processed_ukbb_dir, processed_ld_score_dir, organized_training_data_dir):
	...
	for chromosome in chromosome_arr:
		...
		for study in study_names:
			...
			arr = np.asarray(arr)
			standardized_arr = (arr - np.mean(arr))/np.std(arr)
			chromosome_ukbb.append(np.asarray(standardized_arr))
		...
		for line in f:
			...
			npy_output_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_chi_squared_stats.npy'
...
def pickle_standardized_cluster_level_variant_names(chromosome_arr, processed_ld_score_dir, organized_training_data_dir):
	...
	for chromosome in chromosome_arr:
		...
		for line in f:
			...
			npy_output_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_snp_indices.npy'
...
def pickle_standardized_cluster_level_data(chromosome_arr, processed_ukbb_dir, processed_ld_score_dir, organized_training_data_dir):
	input_study_file = processed_ukbb_dir + 'updated_ukbb_studies.txt'
	temp_data = np.loadtxt(input_study_file,dtype=str,delimiter='\t')
	study_names = temp_data[1:,0]
	sample_sizes = temp_data[1:,2]
	pickle_standardized_cluster_level_ukbb_data(chromosome_arr, study_names, processed_ukbb_dir, processed_ld_score_dir, organized_training_data_dir)
	pickle_standardized_cluster_level_pairwise_ld_data(chromosome_arr, processed_ld_score_dir, organized_training_data_dir)
	pickle_standardized_cluster_level_variant_names(chromosome_arr, processed_ld_score_dir, organized_training_data_dir)
	aggregate_cluster_file = organized_training_data_dir + 'usldsc_training_standardized_snp_cluster_files.txt'
	...
	for chromosome in chromosome_arr:
		...
		for line in f:
			...
			cluster_ukbb_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_chi_squared_stats.npy'
			cluster_pairwise_ld_matrix_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_pairwise_ld_correlation_mat.npy'
			cluster_variant_names_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_snp_indices.npy'
			cluster_pairwise_ld_neighbor_positions_file = organized_training_data_dir + 'usldsc_training_chr_standardized_' + chromosome + '_' + cluster_id + '_pairwise_ld_neighbor_positions.npy'
...
pickle_standardized_ukbb_chi_squared_stats(chromosome_arr, processed_ukbb_dir, organized_training_data_dir)
...
pickle_standardized_cluster_level_data(chromosome_arr, processed_ukbb_dir, processed_ld_score_dir, organized_training_data_dir)
